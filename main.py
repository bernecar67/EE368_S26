...
def logged_in():
    ...
    if not session.get('logged_in'):
        ...
        if user:
            if user.get('picture'):
                return render_template('loggedIn.html', name=user['name'], email=user['email'], profile_pic=user['picture'])
            else:
                return render_template('loggedIn.html', name=user['name'], email=user['email'],profile_pic=user.get('avatar_url'))
        else:
            return redirect(url_for('login'))
    else:
        return render_template('loggedIn.html', name=session['user'], email=session['email'],profile_pic="https://images.ctfassets.net/qyg0c6wmz8bn/4cggfPN1EksMGXkMl9oAv8/6a1a7ac4c8fac7968c70076bf9ef71a3/computer-science-vs-software-engineering.jpg?w=1920&q=75&fm=webp")
...
def google_authorize():
   ...
   try:
       ...
       return redirect(url_for('login', oauth_error=f"Unexpected error occurred while authorizing. Please see the following: {e}"))
   ...
   try:
       ...
       return redirect(url_for('login', oauth_error=f"Unexpected error occurred while fetching user info. Please see the following: {e}"))
   user_info = resp.json()
   session['user'] = user_info
   print(session)
   return redirect('/loggedIn')

# Custom redirect to Custom OAuth
@app.route('/custom')
def custom_redirect():
   try:
       return custom_oauth_server.authorize_redirect(url_for('custom_authorize', _external=True))
   except requests.exceptions.Timeout:
        return redirect(url_for('login', oauth_error="Connection timed out. Please try again."))
   except requests.exceptions.ConnectionError:
        return redirect(url_for('login', oauth_error="Connection error. Please try again."))
